...
Noesis (including all associated scripts, plugins, and tools) may not be used in any way for the development of Blender scripts, importers, etc. unless I have given my explicit permission. Be aware that open source software is used very insidiously in today's culture of capitalist exploitation. Recognize projects which enjoy the support of many large corporations, and understand that the real reasons for that support are not altruistic. Choose to support small projects instead, and don't contribute to software monopolies.

Instead of cannibalizing functionality for Blender so that Microsoft, Facebook, and Activision can laugh at you from on high (in between large gulps of cheap artist/engineer labor), consider helping Noesis! There are good reasons that Noesis isn't open-sourced in this capitalist hellscape, but it is free software in the truest sense. Its aim is to provide knowledge and insight, which is why it's named Noesis. To that end, you can help too. Maybe try stopping by the XeNTaX Discord to share in the knowledge. Chances are, if you're considering using your time to rip functionality out of Noesis instead of doing some real reverse engineering, the community has a lot it can teach you.
...
==========
Version 4.474:
 - Added an asynchronous desktop capture interface, exposed to both native and Python API's.
 - Added a script which utilizes the desktop capture interface to serve a Motion JPEG stream via HTTP. See optionalplugins/python/tool_mjpegserver.py.
 - Limited debugger support in both native and Python API's. See registerDebuggerDataHandler, registerDebuggerTickCallback, etc.
 - Debugger support has been integrated into the Jaguar CRY script. Memory can now be interpreted as CRY data directly from the debugger.
 - Debugger support has been integrated into the Cybermorph script. Memory can now be interpreted as a Cybermorph model/image.
 - Added a "use texture name as label" option in the data viewer.

Version 4.473:
 - Support for compressing and decompressing SA-style Sega Channel images. (this will save some more game history) See optionalplugins/python/fmt_segachannel_sa.py.
 - Fixed a bug with large-format symbols in the COFF dumper.
 - Added rapi.imageTwiddlePSP.
 - Added rapi.decompSegaChanSA, rapi.getSegaChanSASize, rapi.compressSegaChanSA.

Version 4.472:
 - Added a kCF_HostImplementsStepOver flag for the Universal Debugger protocol. If the host doesn't to elect to implement the "step over" state for a given CPU context, the client implements it with a temporary breakpoint.
 - Added a "Run to Cursor" right-click context menu item for the UD disassembly view.
 - Made a few minor core changes to the UD protocol, bumped the protocol version. (make sure you're using the latest BigPEmu)

Version 4.471:
 - More Universal Debugger polish.

Version 4.47:
 - Atari Jaguar BPEG image support. See optionalplugins/python/fmt_atarijaguar_bpeg.py.
 - Lots of new features in the Universal Debugger. This is mainly in service to BigPEmu, my Atari Jaguar emulator.
 - Somehow, I've lived long enough after my cancer diagnosis to update Noesis again.

Version 4.466:
 - Support for Chulip (PS2) character models, zone models, room models, animations, and so on. See optionalplugins/python/fmt_chulip_bin.py. This script was initially written under contract, but my wonderful client has given me permission to adapt it for public release. You'll find it's better commented and cleaner than a lot of the junk I toss in here. I couldn't be lazy, it had to be readable for the client! It also covers a lot of PS2 texture/palette functionality which I often see people getting wrong, so please do use it to educate yourself.

Version 4.465:
 - Lots and lots of bug fixes.
 - Added -imgbicubicmin and -imgtcont.
 - Some other stuff that I can't be bothered to filter out from 8 months of changelists. There's also a Jaguar RISC disassembler in here now, although it's only really here for my personal use and follows some unorthodox conventions.

Version 4.464:
 - Added rapi.imagePS2WriteAndReadback32.
 - Added Image_InstancedCachedRgb24ToRgba32 and Image_ClearInstancedCache.
...
Version 4.461:
 - Added support for Blade (PSX) and Jinx (PSX) to the Quake II PSX importer. (same engine) This includes animation support.
 - Added support for the "RAPO" Quake MDL variant.
 - Added support for some more obscure PCX format variations.
 - Added NoePreserveRapiContext and implemented it in noewin. This makes it safe for scripts to change the current rapi module inside of control callbacks.
 - Added a couple of extra palette.lmp search paths for the Q1BSP importer.
 - Added a "Hidden items in shellview" option in the data viewer.
 - Fixed shell view refreshing unnecessarily in some cases when a file is loaded via script.
 - Bumped NOESIS_PLUGINAPI_VERSION to 82.

Version 4.46:
 - Added support for TMNT (arcade) and TMNT:TiT (arcade), see optionalplugins/python/fmt_tmnt_arcade.py.
 - Added rapi.getInternalViewData and rapi.setInternalViewData, this is mainly intended to allow tools to preserve view settings across scene changes.
 - Added -bonemapex, exports a bone map (.bma) file along with the model if a skeleton is present.
 - Added -gltfdrawunref for the glTF importer, draws meshes even if they aren't referenced by the scene.
 - Added rapi.setSelectedDirectory at blackninja's request.
 - Fixed another potential bug in the ELF loader and added rapi.elfLoaderResetLoadedSectionSize.

Version 4.459:
 - Added rapi.imageUntileAMDRDNA, rapi.imageTileAMDRDNA, and rapi.imageCalculateAMDRDNAMipInfo.
 - Formally exposed the ELF loader through the rapi module and fixed some problems with 64-bit ELF support.
...
Noesis (including all associated scripts, plugins, and tools) may not be used in any way for the development of Blender scripts, importers, etc. See the note near the top of this document.
