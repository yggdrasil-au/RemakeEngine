...
class Console:
    ...
    def log(*objects, sep: str = ' ', end: str = '\n', file: Optional[TextIO] = None, flush: bool = False) -> None:
        ...
        """
        py.print(*objects, sep=sep, end=end, file=file, flush=flush)
...
def cprint(colour: str, message: str) -> None:
    ...
    """
    py.print(f"{colour}{message}{Colours.RESET}", file=sys.stdout)
...
some built-in functions are not provided in the override and require the use of py.print() to use
this is a non issue as the built-in.print() is still available
"""
def print(colour: str | None = "", prefix: str | None = "", prefixcolour: str | None = "", msg: str | None = None, fileout: Optional[TextIO] = sys.stdout, flush: Optional[bool] = False) -> None:
...
def printy(
    ...
    if message is None or message == "":
        py.print("", file=fileout, flush=flushParam)
    elif colour not in Colours.__dict__.values():
        py.print(message, file=fileout, flush=flushParam)
    else:
        py.print(f"{colour}{message}{Colours.RESET}", file=fileout, flush=flushParam)
