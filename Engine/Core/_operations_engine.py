# Engine\Core\_operations_engine.py
import json
import logging
import subprocess
import time
...
class OperationsEngine:
    ...
    def _resolve_placeholders(self, value, context) -> dict[Any, Any] | list[Any] | Any | str:
        """
                ...
                if awaiting_input and process.poll() is None and process.stdin:
                    try:
                        if stdin_provider is not None:
                            ...
                            answer = py.input((last_prompt_message or "Input: ") + " ")
                    except Exception:
                        answer = None
