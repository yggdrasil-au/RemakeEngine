...
def main(working_dir, preinstanced_dir, blend_dir, glb_dir, output_dir, output_file, root_drive, blank_blend_source, verbose, debug_sleep, export_fbx, export_glb, export_single, marker) -> None:
    ...
    try:
        ...
        init_args.extend(["--output-file", str(output_file)])
        ...
        if verbose == "True":
            init_args.extend(["--verbose", verbose])
...
if __name__ == "__main__":
    ...
    blend_dir = Path(working_dir, "GameFiles", "TEMP_BLENDER")
    glb_dir = Path(working_dir, "GameFiles", "STROUT")
    output_dir = Path(working_dir, "Tools", "Blender")
    output_file = Path(output_dir, "asset_mapping.json")
    ...
    main(working_dir, preinstanced_dir, blend_dir, glb_dir, output_dir, output_file, root_drive, blank_blend_source, verbose, debug_sleep, export_fbx, export_glb, export_single, marker)
