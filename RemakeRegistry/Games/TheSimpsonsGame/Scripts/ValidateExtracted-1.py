...
def check_required_directories(base_folder: str, required_dirs: list) -> bool:
    ...
    """
    print(f"--- Checking for Required Subdirectories in: {base_folder} ---")
    all_found = True
    missing_dirs = []
    found_dirs_count = 0
    ...
    if not all_found:
        print("WARNING: The following required subdirectories were NOT found:")
        for m_dir in missing_dirs:
            print(f"  - {m_dir}")
        print(f"Found {found_dirs_count}/{len(required_dirs)} required subdirectories.")
        print("Continuing with database file checks, but results might be affected by missing base directories.")
    else:
        print(f"✅ All {len(required_dirs)} required subdirectories found.")
    print("-" * 20)
    return all_found
...
def check_file_existence(db_path: str, base_check_folder: str):
    ...
    if not os.path.isdir(base_check_folder):
        print(f"ERROR: Check folder not found or is not a directory: {base_check_folder}")
        return
    ...
    try:
        ...
        print(f"Checking against base folder: {base_check_folder}\n")
        ...
        for table_name, path_column_name in tables_to_check.items():
            ...
            try:
                ...
                print(f"  WARNING: Could not query table {table_name}. Error: {e}")
            ...
            for row_index, row in enumerate(rows):
                ...
                if relative_path is None:
                    print(f"  WARNING: Row {row_index + 1} in {table_name} has a NULL {path_column_name}. Skipping.")
                    continue
                ...
                if not os.path.isfile(full_expected_path):
                    #print(f"  MISSING (DB): {full_expected_path} (from {table_name})")
                    overall_db_files_all_found = False
                    total_db_files_missing += 1
                    table_missing_count +=1
                # else:
                #     print(f"  FOUND (DB): {full_expected_path}") # Optional: for verbose output
            ...
            if table_checked_count > 0: # Only print summary if files were expected
                if table_missing_count == 0:
                    print(f"  All {table_checked_count} files from {table_name} (as per DB) found.")
                else:
                    print(f"  {table_missing_count} file(s) MISSING from {table_name} (out of {table_checked_count} expected in this table).")
            else:
                print(f"  No valid file paths to check in {table_name}.")
            print("-" * 20)

        print("\n--- Overall Summary ---")
        if not required_dirs_all_present:
            print("NOTE: Some required base subdirectories were missing. This might affect the accuracy of the database file checks.")
        
        if total_db_files_checked == 0:
            print("No files were checked from the database. Ensure tables exist, contain data, and paths are valid.")
        elif overall_db_files_all_found:
            print(f"✅ All {total_db_files_checked} files checked from the database were found in {base_check_folder}.")
        else:
            print(f"❌ {total_db_files_missing} out of {total_db_files_checked} files checked from the database are MISSING.")
