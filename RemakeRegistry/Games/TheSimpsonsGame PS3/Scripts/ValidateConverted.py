...
def check_required_directories(base_folder: str, required_dirs: list) -> bool:
    ...
    print(colour="cyan", prefix="Validate", msg="-" * 20)
    return all_found

def check_file_existence(db_path: str, base_check_folder: str):
    if not os.path.isfile(db_path):
        print(colour="red", prefix="Validate", msg=f"ERROR: Database file not found at {db_path}")
        return

    if not os.path.isdir(base_check_folder):
        print(colour="red", prefix="Validate", msg=f"ERROR: Check folder not found or is not a directory: {base_check_folder}")
        return
    ...
    try:
        ...
        print(colour="cyan", prefix="Validate", msg="--- Starting Database File Existence Check ---")
        print(colour="cyan", prefix="Validate", msg=f"Database: {db_path}")
        print(colour="cyan", prefix="Validate", msg=f"Checking against base folder: {base_check_folder}")

        for table_name, path_column_name in tables_to_check.items():
            print(colour="blue", prefix="Validate", msg=f"Checking table: {table_name}...")
            try:
                cursor.execute(f"SELECT {path_column_name} FROM {table_name}")
                rows = cursor.fetchall()
            except sqlite3.OperationalError as e:
                print(colour="yellow", prefix="Validate", msg=f"  WARNING: Could not query table {table_name}. Error: {e}")
            ...
            for row_index, row in enumerate(rows):
                ...
                if relative_path is None:
                    print(colour="yellow", prefix="Validate", msg=f"  WARNING: Row {row_index + 1} in {table_name} has a NULL {path_column_name}. Skipping.")
                    continue
            ...
            if table_checked_count > 0:
                if table_missing_count == 0:
                    ...
                    print(colour="red", prefix="Validate", msg=f"  {table_missing_count} file(s) MISSING from {table_name} (out of {table_checked_count} expected in this table).")
            else:
                print(colour="yellow", prefix="Validate", msg=f"  No valid file paths to check in {table_name}.")
            print(colour="cyan", prefix="Validate", msg="-" * 20)

        print(colour="cyan", prefix="Validate", msg="\n--- Overall Summary ---")
        ...
        if total_db_files_checked == 0:
            print(colour="red", prefix="Validate", msg="No files were checked from the database. Ensure tables exist, contain data, and paths are valid.")
        ...
        print(colour="red", prefix="Validate", msg=f"An unexpected error occurred: {e}")
    finally:
        if conn:
            conn.close()
