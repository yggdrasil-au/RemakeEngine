...
internal sealed class Main : ScriptEngines.IAction {
    ...
    public async Task ExecuteAsync(Core.ExternalTools.JsonToolResolver tools, Core.Services.CommandService commandService, CancellationToken cancellationToken = default) {
        ...
        try {
            ...
            Shared.UI.EngineSdk.PrintLine(message: $"Running js script '{_scriptPath}' with {_args.Length} args...", color: System.ConsoleColor.Cyan);
...
#if DEBUG
            Shared.UI.EngineSdk.PrintLine($"Running js script '{_scriptPath}' with {_args.Length} args...");
