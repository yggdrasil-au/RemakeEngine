...
internal static class InternalToolRegistry {
    private static readonly string ToolsRegistryRoot = Path.Combine(EngineNet.Shared.State.RootPath, "EngineApps", "Registries", "Tools");
        ...
        foreach (string toolDir in Directory.GetDirectories(ToolsRegistryRoot)) {
            ...
            foreach (string jsonFile in Directory.GetFiles(toolDir, "*.json")) {
                try {
                    ...
                    Shared.IO.Diagnostics.Bug($"[InternalToolRegistry] Error parsing registry fragment '{jsonFile}'.", ex);
                    Shared.IO.Diagnostics.Log($"[InternalToolRegistry] Error parsing {jsonFile}: {ex.Message}");
                }
            }
