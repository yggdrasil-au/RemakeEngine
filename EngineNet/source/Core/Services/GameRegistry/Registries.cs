...
internal sealed class Registries {
    ...
    internal static async Task<Registries> CreateAsync() {
        ...
        string gamesRel = System.IO.Path.Combine(EngineNet.Shared.State.RootPath, "EngineApps", "Games");
        string modulesRel = System.IO.Path.Combine(EngineNet.Shared.State.RootPath, Module_registry);

        System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(modulesRel) ?? EngineNet.Shared.State.RootPath);
    ...
    internal Dictionary<string, GameInfo> DiscoverBuiltGames() {
        ...
        foreach (string dir in System.IO.Directory.EnumerateDirectories(_gamesRegistryPath)) {
            ...
            var ctx = new Dictionary<string, object?>(System.StringComparer.OrdinalIgnoreCase) {
                ["Game_Root"] = dir,
                ["Project_Root"] = EngineNet.Shared.State.RootPath
            };
