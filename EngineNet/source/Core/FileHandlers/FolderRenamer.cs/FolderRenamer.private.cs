...
internal static partial class FolderRenamer {
    ...
    private static Dictionary<string, string>? LoadFromDatabase(string dbPath, string tableName, out string description) {
        ...
        try {
            ...
            Shared.IO.Diagnostics.Bug($"[FolderRenamer::LoadFromDatabase()] SQLite error reading '{dbPath}'.", ex);
    ...
    private static Dictionary<string, string>? LoadFromJson(string jsonPath, out string description) {
        ...
        try {
            ...
            Shared.IO.Diagnostics.Bug($"[FolderRenamer::LoadFromJson()] JSON parse error in '{jsonPath}'.", ex);
            WriteError($"Error decoding JSON from {jsonPath}: {ex.Message}");
            return null;
        } catch (System.IO.IOException ex) {
            Shared.IO.Diagnostics.Bug($"[FolderRenamer::LoadFromJson()] IO error reading '{jsonPath}'.", ex);
    ...
    private static bool RenameDirectories(string targetDirectory, IDictionary<string, string> renameMap) {
        ...
        try {
            ...
            Shared.IO.Diagnostics.Bug($"[FolderRenamer::RenameDirectories()] Access denied listing directory '{directoryPath}'.", ex);
            WriteError($"Permission denied when reading directory: {directoryPath}");
            return false;
        } catch (System.IO.IOException ex) {
            Shared.IO.Diagnostics.Bug($"[FolderRenamer::RenameDirectories()] IO error listing directory '{directoryPath}'.", ex);
        ...
        foreach (string itemPath in items) {
            ...
            try {
                ...
                Shared.IO.Diagnostics.Bug($"[FolderRenamer::RenameDirectories()] IO error renaming '{itemPath}' to '{newPath}'.", ex);
                WriteError($"Error renaming '{itemName}' to '{newName}': {ex.Message}");
                skipped += 1;
            } catch (System.UnauthorizedAccessException ex) {
                Shared.IO.Diagnostics.Bug($"[FolderRenamer::RenameDirectories()] Access denied renaming '{itemPath}' to '{newPath}'.", ex);
    ...
    private static string NormalizePath(string path) {
        try {
            ...
            Shared.IO.Diagnostics.Bug("[FolderRenamer::NormalizePath()] Failed to normalize path.", ex);
            return path;
        }
    }
    ...
    private static void TraceError(string message) {
        try {
            ...
            Shared.IO.Diagnostics.Bug("[FolderRenamer::TraceError()] Failed to write diagnostic message.", ex);
