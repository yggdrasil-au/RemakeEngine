...
internal sealed class SegmentScanner {
    ...
    internal (List<Segment> segments, int totalTextures) CollectSegments() {
        ...
        utils.Log.Blue($"  Found {totalEofPatterns} occurrences of EOF_SIGNATURE pattern in the entire file.");
        if (totalEofPatterns != 1) {
            if (totalEofPatterns == 0) {
                throw new Sys.TxdExportException("  ERROR: EOF pattern not found in the file. This may indicate a corrupted or incomplete TXD file.");
            }
        ...
        if (utils.Util.StartsWith(data, 0, SigFileStart)) {
            ...
            utils.Log.Yellow("  File does not start with sig_file_start (0x16). Will scan for 0x14 blocks from beginning.");
            searchPtr = 0;
        }
