...
internal static class TextureFormatConverter {
    internal static ConversionResult Convert(
        ...
        if (actualMipDataSize <= expectedBaseSize && mipMapCountFromFile > 1) {
            utils.Log.Debug($"        WARNING: TXD specifies {mipMapCountFromFile} mipmaps, but payload size ({actualMipDataSize} bytes) only covers the base texture. Forcing mipmaps to 1.");
            safeMipMapCount = 1;
        }
        ...
        if (fmtCode == 0x52) {
            ...
            needsUnswizzle = true;
            utils.Log.Debug($"        Swizzled BGRA format detected. Size: {actualMipDataSize} bytes.");
            ...
            needsUnswizzle = true;
            utils.Log.Debug($"        Swizzled A8 or P8A8 format detected. Size: {actualMipDataSize} bytes.");
