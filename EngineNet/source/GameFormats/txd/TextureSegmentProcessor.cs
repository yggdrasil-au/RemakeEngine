...
internal sealed class TextureSegmentProcessor {
    ...
    internal int ProcessSegment(Segment segment, string outputDir, string outputExtension = "dds") {
            ...
            if (i + NameSignatureLength <= segmentData.Length && utils.Util.StartsWith(segmentData, i, NameSignature)) {
                ...
                if (nameEndScan < segmentData.Length - 1 && segmentData[nameEndScan] == 0x00 && segmentData[nameEndScan + 1] == 0x00) {
                    ...
                    if (totalPixelDataSize == 0) {
                        throw new Sys.TxdExportException($"          FATAL ERROR: Invalid metadata (Size:{totalPixelDataSize} with W:{width}, H:{height}) for '{currentName.Name}' (File Offset: 0x{currentName.OriginalFileOffset:X}).");
                    }
        ...
        return currentName is not null && !currentName.ProcessedMeta
            ? throw new Sys.TxdExportException($"  WARNING: End of segment reached. Pending name '{currentName.Name}' (sig at file 0x{currentName.OriginalFileOffset:X}) did not find or complete its metadata processing.")
            : texturesFound == 0
            ? throw new Sys.TxdExportException($"  No textures successfully exported from segment starting at file offset 0x{segmentOriginalStartOffset:X} that met all processing criteria.")
            : texturesFound;
    }
}
