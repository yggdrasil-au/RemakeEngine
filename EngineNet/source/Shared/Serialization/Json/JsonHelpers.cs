...
public sealed class JsonHelpers {
    ...
    public static Dictionary<string, object?> LoadJsonFile(string filePath) {
        try {
            ...
            Shared.IO.Diagnostics.Bug($"JSON parsing error for file '{filePath}': {ex}");
        } catch (System.IO.IOException ex) {
            Shared.IO.Diagnostics.Bug($"IO error reading file '{filePath}': {ex}");
        } catch (System.UnauthorizedAccessException ex) {
            Shared.IO.Diagnostics.Bug($"Access denied reading JSON file '{filePath}': {ex}");
        } catch (System.ArgumentException ex) {
            Shared.IO.Diagnostics.Bug($"Invalid JSON file path '{filePath}': {ex}");
        }
