...
public sealed class CommandBuilder
{
        ...
        IDictionary<string, object?> promptAnswers)
    {
        ...
        if (op.TryGetValue("prompts", out var promptsObj) && promptsObj is IList<object?> prompts)
        {
            ...
            {
                if (p is not IDictionary<string, object?> prompt) continue;
                ...
                switch (type)
                {
                    ...
                    case "checkbox":
                        ...
                        {
                            parts.Add(sp);
                            foreach (var it in items) if (it is not null) parts.Add(it.ToString()!);
                        }
                        break;
                    case "text":
                        var v = ans?.ToString();
