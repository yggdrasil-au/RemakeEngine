...
public static class TxdExtractor {
    ...
    public static Boolean Run(IList<String> args) {
        try {
            ...
            exporter.ExportPath(options.InputPath, options.OutputDirectory);
            return true;
    ...
    private static Byte[] CreateDdsHeaderRgba(Int32 width, Int32 height, Int32 mipMapCount) {
        ...
        writer.Write(0);
        writer.Write(32);
        writer.Write(unchecked((Int32)0x000000FF));
        writer.Write(unchecked((Int32)0x0000FF00));
        writer.Write(unchecked((Int32)0x00FF0000));
    ...
    private sealed class NameInfo {
        ...
        public Int32 NameSigOffsetInSegment { get; }
        public Int32 OriginalFileOffset { get; }
        public Boolean ProcessedMeta { get; private set; }

        public void MarkProcessed() => ProcessedMeta = true;
    }
    ...
    private readonly struct ConversionResult {
        ...
        public Boolean NeedsUnswizzle { get; }
    ...
    private sealed class SegmentScanner {
        ...
        public (List<Segment> segments, Int32 totalTextures) CollectSegments() {
            ...
            List<Int32> eofOccurrences = new List<Int32>();
            Int32 searchIdx = 0;
            while (true) {
    ...
    private sealed class TextureSegmentProcessor {
        ...
        private static readonly HashSet<Byte> KnownFormatCodes = new() { 0x52, 0x53, 0x54, 0x86, 0x02 };
    ...
    private sealed class TxdExporter {
        ...
        private Int32 ExportTexturesFromTxd(String txdFilePath, String outputDirBase) {
            ...
            SegmentScanner scanner = new SegmentScanner(data, txdFilePath);
    ...
    private static class Log {
        ...
        public static void Cyan(String message) => Write(ConsoleColor.Cyan, message);
        public static void Blue(String message) => Write(ConsoleColor.Blue, message);
        public static void Green(String message) => Write(ConsoleColor.Green, message);
        public static void Yellow(String message) => Write(ConsoleColor.Yellow, message);
        public static void Red(String message) => Write(ConsoleColor.Red, message, true);
        public static void Gray(String message) => Write(ConsoleColor.DarkGray, message);
    ...
    private static Int32 CountOccurrences(ReadOnlySpan<Byte> data, ReadOnlySpan<Byte> pattern) {
        ...
        while (index <= data.Length - pattern.Length) {
            Int32 found = data.Slice(index).IndexOf(pattern);
            if (found == -1)
                break;
            count += 1;
            index += found + 1;
    ...
    private static Int32 IndexOf(ReadOnlySpan<Byte> data, ReadOnlySpan<Byte> pattern, Int32 start) {
        ...
        if (start < 0 || start > data.Length - pattern.Length)
            return -1;
        Int32 pos = data.Slice(start).IndexOf(pattern);
    ...
    private static Boolean StartsWith(ReadOnlySpan<Byte> data, Int32 offset, ReadOnlySpan<Byte> pattern) {
        return pattern.IsEmpty
            ? true
            : offset < 0 || offset + pattern.Length > data.Length ? false : data.Slice(offset, pattern.Length).SequenceEqual(pattern);
    }
}
