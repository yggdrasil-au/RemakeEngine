...
internal static partial class Main {
    ...
    public static bool Run(List<string> args) {
        try {
            ...
            // --- CHANGED ---
            Core.Utils.EngineSdk.SdkConsoleProgress.ActiveProcess? currentJob = null;
            using System.Threading.CancellationTokenSource cts = new();
            System.Threading.Tasks.Task progress = Core.Utils.EngineSdk.SdkConsoleProgress.StartPanel(
                total: files.Count,
                ...
                activeSnapshot: () => currentJob is null ? new List<Core.Utils.EngineSdk.SdkConsoleProgress.ActiveProcess>() : new List<Core.Utils.EngineSdk.SdkConsoleProgress.ActiveProcess> { currentJob },
            ...
            foreach (string txdFile in files) {
                try {
                    // --- CHANGED ---
                    currentJob = new Core.Utils.EngineSdk.SdkConsoleProgress.ActiveProcess { Tool = "txd", File = System.IO.Path.GetFileName(txdFile), StartedUtc = System.DateTime.UtcNow };
                    // --- END CHANGED ---
...
#if DEBUG
                System.Console.WriteLine("[TxdExtractor] Progress task cancelled.");
#endif
                /* ignore */
            }
            return true;
