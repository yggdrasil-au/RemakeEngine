...
# Define the expected ending patterns for the two types
...
def process_mus_file(filepath):
  """
  Reads a .mus file, determines its type based on header suffix,
  and returns its header, file data, and type for combined analysis.
  Prints individual header info and detected type.

  Returns:
      A tuple: (header_bytes, file_data_bytes, file_type_str) or (None, None, None) on error.
  """
  ...
  try:
    with open(filepath, 'rb') as f:
      ...
      if len(header) < HEADER_SIZE:
          print(f"Warning: File is smaller than header size ({len(header)} bytes).")
          # Still return data for basic analysis if needed
          f.seek(0)
          file_data = f.read()
          print(f"Header ({len(header)} bytes): {header.hex(' ').upper()}")
          return header, file_data, file_type # Return as unknown type


      print(f"Header ({HEADER_SIZE} bytes): {header.hex(' ').upper()}")
      signature = header[:4] # First 4 bytes as signature/ID
      header_suffix = header[12:16] # Last 4 bytes of the 16-byte header

      print(f"Signature/ID: {signature.hex(' ').upper()}")

      # Determine file type based on header suffix
      if header_suffix == HEADER_TYPE1_SUFFIX:
          file_type = "Type 1"
      elif header_suffix == HEADER_TYPE2_SUFFIX:
          file_type = "Type 2"

      print(f"Detected Type: {file_type} (based on header ending {header_suffix.hex(' ').upper()})")


      # Read the rest of the file for further analysis
      f.seek(0) # Reset file pointer
      file_data = f.read()
      return header, file_data, file_type
...
if __name__ == "__main__":
  ...
  if not mus_files:
    ...
    file_types_counter = collections.Counter() # To count file types

    # Process each file and aggregate data
    for mus_file in mus_files:
      header, file_data, file_type = process_mus_file(mus_file)
      if header and file_data and file_type:
          total_bytes_processed += len(file_data)
          file_signatures[header[:4]] += 1 # Count unique IDs/signatures
          file_types_counter[file_type] += 1 # Count detected types
    ...
    # File Types Found
    print("\nFile Types Found:")
    for ftype, count in file_types_counter.items():
        suffix = ""
        if ftype == "Type 1":
            suffix = f"(ends with {HEADER_TYPE1_SUFFIX.hex(' ').upper()})"
        elif ftype == "Type 2":
            suffix = f"(ends with {HEADER_TYPE2_SUFFIX.hex(' ').upper()})"
        print(f"  {ftype}: {count} file(s) {suffix}")


    # Signatures Found
    print("\nUnique File Signatures/IDs Found (First 4 Bytes):")
    # Only show first few if there are many, or consider grouping by type if needed
    if len(file_signatures) > 10:
         print(f"  {len(file_signatures)} unique signatures found (showing first 10):")
         for i, (sig, count) in enumerate(file_signatures.most_common()):
             if i >= 10: break
             print(f"    Signature {sig.hex(' ').upper()}: {count} file(s)") # Should always be 1 with current data
    else:
