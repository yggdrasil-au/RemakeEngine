...
public class ToolsJsonSchemaTests {
    ...
    private static void ValidatePlatformOrMeta(String toolName, String version, JsonProperty child, List<String> problems) {
        ...
        if (NonPlatformKeys.Contains(key)) {
            ...
            if (val.ValueKind == JsonValueKind.Object && val.TryGetProperty("url", out JsonElement u) && u.ValueKind == JsonValueKind.String)
                return; // allow object with url
            problems.Add($"{toolName}@{version} key '{key}' must be a string or an object with 'url'.");
            return;
        }

        // Only validate as a platform if key has OS prefix
        String lower = key.ToLowerInvariant();
        Boolean looksLikePlatform = lower.StartsWith("win") || lower.StartsWith("linux") || lower.StartsWith("mac") || lower.StartsWith("macos");
        if (looksLikePlatform && val.ValueKind == JsonValueKind.Object && val.TryGetProperty("url", out _)) {
            ValidatePlatformBlock(toolName, version, key, val, problems);
        }
        // otherwise: ignore keys like 'web-editor', 'export-templates' so they can be grouping/meta
    }
