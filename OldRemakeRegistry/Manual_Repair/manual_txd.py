...
def main():
    texture_map_dir = input("Enter texture_map_dir: ").strip()
    asset_uuid = input("Enter asset_uuid: ").strip()

    metadata_file = os.path.join(texture_map_dir, "blend_metadata.json")
    json_path = os.path.join(texture_map_dir, "texture_export.json")

    # Check if files exist
    for file_path in [metadata_file, json_path]:
    ...
    output = {
        "uuid": uuid,
        "texture_map_dir": texture_map_dir,
